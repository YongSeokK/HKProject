...
for num in Data_Json.keys():
    ...
    # DB 쿼리문
    query = "INSERT INTO food_recipe ({}) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)".format(col_Text)
    Result_Text = (
        Row_Dict['레시피일련번호'], Row_Dict['요리명'], Row_Dict['등록자명'], Row_Dict['조회수'], Row_Dict['추천수'], Row_Dict['요리상황별 명'],
        Row_Dict['요리소개'], Row_Dict['요리재료내용'], Row_Dict['요리인분 명'], Row_Dict['요리난이도 명'], Row_Dict['요리시간 명'])
