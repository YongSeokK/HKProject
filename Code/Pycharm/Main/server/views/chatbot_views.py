...
from ..define.navershop_api import Navershop
...
def find_img():
    ...
    if len(Folder_List) >= 2:
        ret = {"version": "2.0", "template": {"outputs": [{"simpleText": {"text": '사진을 1장만 보내주세요'}}],
                                              "quickReplies": [{"messageText": "이미지 검색",
                                                                "action": "message",
                                                                "label": "이미지 검색"}]
                                              }}
    else:
        ...
        if len(Yolorun_return) >= 2:
            ret = {"version": "2.0", "template": {"outputs": [{"simpleText": {"text": '음식 1개만 있는 사진을 보내 주세요.'}}],
                                                  "quickReplies": [{"messageText": "이미지 검색",
                                                                    "action": "message",
                                                                    "label": "이미지 검색"}]
                                                  }}
        elif len(Yolorun_return) == 0:
            ret = {"version": "2.0", "template": {"outputs": [{"simpleText": {"text": '확인할 수 있는 음식이 없습니다.'}}],
                                                  "quickReplies": [{"messageText": "이미지 검색",
                                                                    "action": "message",
                                                                    "label": "이미지 검색"}]
                                                  }}
            ...
            # ret은 카톡챗봇의 응답
            ret = {"version": "2.0",
                   "template": {"outputs": [{"simpleText": {"text": Yolorun_return[0] + " 사진으로 검색 합니다."}},
...
def find_ingredients():
    ...
    rets = {"version": "2.0",
            "template": {"outputs": [{"carousel": {"type": "listCard",
                                                   "items": Carousel_List}}], }}
    return jsonify(rets)
    print('완료')
...
def find_txt():
    ...
    # ret은 카톡챗봇의 응답
    ret = {"version": "2.0", "template": {"outputs": [{"simpleText": {"text": temp + "으로 검색합니다"}},
                                                      {"carousel": {"type": "itemCard",
                                                                    "items": textchat_List}}],
                                          "quickReplies": [{"messageText": "이미지 검색",
                                                            "action": "message",
